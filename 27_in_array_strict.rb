...
# * If strict is false, in_array_strict? should not care about LEtTeR cASinG and should return true for any match .downcase
...
def in_array_strict? (string, arr, strict)
    ...
    arr.each do |element|
        ...
        if strict == true and element == string 
            loopster = true
        elsif strict == false && element.downcase ==  string.downcase
            loopster = true
        end
    end
    return loopster
